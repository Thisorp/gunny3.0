...
namespace Game.Logic.Phy.Object
{
    public class Living : Physics
    {
        ...
        protected int MakeDamage(Living target)
        {
            ...
            if (target.AddArmor && (target as Player).SecondWeapon != null)
            {
                int DamageReduct = (target as Player).SecondWeapon.Template.Property7 + (int)Math.Pow(1.1, (double)(target as Player).SecondWeapon.StrengthenLevel);
                baseGuard += (double)DamageReduct;
                defence += (double)DamageReduct;
            }
            if (IgnoreArmor)
            {
                baseGuard = 0;
