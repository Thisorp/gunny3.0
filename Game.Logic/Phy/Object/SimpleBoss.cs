    ...
    public class SimpleBoss : TurnedLiving
    {
        ...
        public void CreateChild(int id, Point[] brithPoint, int maxCount, int maxCountForOnce, int type)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                this.CreateChild(id, brithPoint[num2].X, brithPoint[num2].Y, 4, maxCount, 1);
            }
        ...
        public void CreateChild(int id, int x, int y, int disToSecond, int maxCount, int direction)
        {
            if (this.CurrentLivingNpcNum < maxCount)
            {
                ...
                {
                    this.Child.Add(((PVEGame)base.Game).CreateNpc(id, x + disToSecond, y, direction));
                    this.Child.Add(((PVEGame)base.Game).CreateNpc(id, x, y, direction));
                    return;
                }
