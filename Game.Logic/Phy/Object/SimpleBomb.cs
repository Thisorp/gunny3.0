                   ...
                   case BombType.CURE:
                        foreach (Living p in playersAround)
                        {
                            ...
                            if (playersAround.Count == 1)
                                plus = 1;
                            if (playersAround.Count == 2)
                                plus = 0.75;
                            if (playersAround.Count == 3)
                                plus = 0.5;
                            if (playersAround.Count == 4)
                                plus = 0.375;
                            if (playersAround.Count > 4)
                                plus = 0.3;
                            int blood = 0;                            
                            blood = (int)(((Player)m_owner).PlayerDetail.SecondWeapon.Template.Property7 * Math.Pow(1.1, ((Player)m_owner).PlayerDetail.SecondWeapon.StrengthenLevel) * plus);
                            p.AddBlood(blood);
                            if (p is Player)
                            {
