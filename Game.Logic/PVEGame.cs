...
namespace Game.Logic
{
    public class PVEGame : BaseGame
    {
        ...
        public override Player RemovePlayer(IGamePlayer gp, bool isKick)
        {
            ...
            if (player != null)
            {
                player.PlayerDetail.RemoveGP(gp.PlayerCharacter.Grade * 0);
                string msg = null;
                string msg1 = null;
                if (player.IsLiving && GameState == eGameState.Playing)
                {

                    msg = LanguageMgr.GetTranslation("AbstractPacketLib.SendGamePlayerLeave.Msg4", gp.PlayerCharacter.Grade * 0);
                    msg1 = LanguageMgr.GetTranslation("AbstractPacketLib.SendGamePlayerLeave.Msg5", gp.PlayerCharacter.NickName, gp.PlayerCharacter.Grade * 0);
